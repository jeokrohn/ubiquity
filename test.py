...
from simple_unifi import UnifiAPI, PortConf, UbiBase, PortOverride
...
class TestApp(cmd2.Cmd):
    ...
    parser.add_argument(
        "logging", choices=["on", "off"], help='turn logging of requests "on" or "off"'
    )
    ...
    def do_clear_cookies(self, _: argparse.Namespace):
        """
    ...
    @cmd2.with_category("SYS")
    def do_show_cookies(self, _: argparse.Namespace):
        """
    ...
    def do_extra(self, args: argparse.Namespace):
        ...
        if args.extra:
            ...
            enum_extra = next(
                e for e in {"allow", "forbid", "ignore"} if e == args.extra
            )
            UbiBase.model_config["extra"] = enum_extra
        self.poutput(f"Handling of extra attributes: {UbiBase.model_config['extra']}")

    @cmd2.with_category(APP_CATEGORY)
    def do_system(self, _: argparse.Namespace):
        """
        call api/system
        """
    ...
    @cmd2.with_category(APP_CATEGORY)
    def do_login(self, _: argparse.Namespace):
        """
        call api/auth/login
        """
    ...
    def do_stat_device(self, args: argparse.Namespace):
        ...
        if args.summary:
            ...
            self.poutput(
                "\n".join(
                    f"mac: {d.mac} id: {d.id}, ip: {d.ip:{ip_len}} type: {d.type:{type_len}} "
    ...
    def do_poe_ports(self, args: argparse.Namespace):
        ...
        devices_with_poe = [
            ...
            if (
                poe_ports := [
                    port for port in d.port_table if port.port_poe and port.poe_enable
                ]
            )
        ]
        for device, port_list in devices_with_poe:
            self.poutput(
                f"mac: {device.mac} id: {device.id}, ip: {device.ip} type: {device.type} "
                f"model: {device.model} ports: {len(device.port_table)}"
            )
            po_dict = device.override_dict
            for port in port_list:
                self.poutput(
                    f"  port: {port.port_idx} host: {port.mac_table[0].hostname}"
                )
    ...
    parser.add_argument(
        ...
        "If missing then consider all ports",
        nargs="?",
    )
    parser.add_argument(
        "mode",
    ...
    def do_poe_mode(self, args: argparse.Namespace):
        ...
        poe_ports = [port.port_idx for port in device.port_table if port.port_poe]
        updated = False
        if not poe_ports:
        ...
        if args.port:
            try:
                ...
                return
            target_ports = [port for port in poe_ports if port_re.match(str(port))]
        else:
        ...
        for target_port in target_ports:
            port = next(
                (
                    ...
                    if port.port_idx == target_port
                ),
                None,
            )
            if port is None:
                self.perror(
                    f"Invalid port index: {target_port}. Allowed: "
            ...
            self.poutput(
                f"port {port.port_idx}, PoE mode: {mode if mode is not None else 'unknown'}{change}"
            )
        ...
        if updated:
            ...
            self.api.apply_port_overrides(
                device_id=device.id, overrides=device.port_overrides
            )

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "mode", type=str, choices=["auto", "off"], help="desired PoE mode. "
    )
    ...
    def do_poe_test(self, args: argparse.Namespace):
        ...
        :return:
        """
        self.api.change_poe_mode(
            mac="74:ac:b9:10:7b:8a", mode=args.mode, ports=[11, 12, 13, 14]
        )
...
def main():
    ...
    if not all((user, password, host)):
        print(
            "UNIFI_USER, UNIFI_PASSWORD, and UNIFI_HOST all need to be set",
            file=sys.stderr,
        )
        exit(1)
    if True:
