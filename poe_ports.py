...
class PoeManager(hass.Hass):
    ...
    def initialize(self):
        ...
        if missing:
            log(f"missing parameters {', '.join(missing)}", level="ERROR")
            return
        # set all arguments
    ...
    def parse_args(s: str) -> dict[str, str]:
        ...
        arg_parser = re.compile(
            r"""([^=]+) # key: sequence of characters other than "="
    ...
    def parameter_change(self, entity, attribute, old_value, new_value: str, *args):
        ...
        log(
            f"parameters: entity={entity}, attribute={attribute}, old={old_value}, new={new_value}"
        )
        ...
        if mode not in ["auto", "off"]:
            log("invalid parameter. Allowed: auto, off", level="ERROR")
            return

        # get API instance
        with UnifiAPI(
            host=self.unifi_host,
            user=self.unifi_user,
            password=self.unifi_pass,
            verify=False,
        ) as api:
